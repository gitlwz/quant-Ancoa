...
function generateOHLC(count) {
    ...
    for (var i = 0; i < count; i++) {
        ...
        for (var j = 0; j < 4; j++) {
            boxVals[j] = (Math.random() ) * dayRange + 2000;
        }
...
class Member extends Component {
    ...
    componentDidMount = () => {
        this.myChart = echarts.init(document.getElementById(`marketview-dayline-view`))
        this.myChart.setOption(option)
        this.resize = this.myChart.resize;
        let key = "dayline";
        const { dispatch, echartsArray } = this.props;
        let _echarts = echartsArray.find((ele) => ele.key == key);
        if (!!_echarts) {
    ...
    render() {
        ...
        return (
            <div id="marketview-dayline-view" style={{ width: "100%", height: `100%` }}>
            </div>
            );
    }
}
export default connect(({ marketview }) => {
    const { echartsArray } = marketview;
    return {
        echartsArray
    }
